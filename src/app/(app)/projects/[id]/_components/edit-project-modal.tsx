...
import {
  RiCloseLine, RiArrowDownSLine, RiArrowUpSLine, RiCheckLine,
...
function parseSprints(sprint: string | null): string[] {
  if (!sprint) return [];
  return sprint.split(",").map((s) => s.trim()).filter(Boolean);
}
...
function OwnerPicker({ members, loading, value, onChange }: {
        ...
        <button type="button" className={triggerClass}>
          <span className="flex items-center gap-2.5 min-w-0">
            {value ? (
              ...
              <><span className="shrink-0" style={{ width: 4, height: 16, borderRadius: 9999, backgroundColor: "#DEE8E8" }} /><span style={{ color: "#8D8D8D", fontWeight: 500 }}>{loading ? "Loading…" : "Select owner…"}</span></>
            )}
          </span>
...
function AssigneesPicker({ members, loading, value, onChange }: {
  members: WorkspaceMember[]; loading: boolean;
  value: WorkspaceMember[]; onChange: (m: WorkspaceMember[]) => void;
}) {
  const [open, setOpen] = React.useState(false);
  const triggerClass = [
    "h-10 w-full inline-flex items-center justify-between gap-3 rounded-full border px-[18px] text-sm outline-none transition-colors",
  ...
  function toggle(m: WorkspaceMember) {
    value.find((v) => v.id === m.id) ? onChange(value.filter((v) => v.id !== m.id)) : onChange([...value, m]);
  }
  return (
    <Popover.Root open={open} onOpenChange={setOpen}>
      <Popover.Trigger asChild>
        <button type="button" className={triggerClass}>
          <span className="flex items-center gap-2 min-w-0 flex-1">
            {value.length === 0 ? (
              <>
                <span className="shrink-0" style={{ width: 4, height: 16, borderRadius: 9999, backgroundColor: "#DEE8E8" }} />
                <span style={{ color: "#8D8D8D", fontWeight: 500 }}>{loading ? "Loading…" : "Select assignees…"}</span>
              </>
            ) : (
              <>
        ...
        <Popover.Content sideOffset={4} align="start" avoidCollisions onOpenAutoFocus={(e) => e.preventDefault()}
          ...
          {value.length > 0 && (
            <div style={{ padding: "8px 12px", borderTop: "1px solid #F0F7F7" }}>
              <button type="button" onClick={() => onChange([])} className="text-xs font-medium text-foreground-muted hover:text-red-500 transition-colors">
      ...
      {STATUS_OPTIONS.map((opt) => {
        ...
        return (
          <button key={opt.value} type="button" onClick={() => onChange(opt.value)}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium border transition-colors ${isActive ? s.active : "border-border text-foreground hover:border-foreground-muted"}`}>
...
export function EditProjectModal({ open, project, onClose, onSaved, onDeleted }: EditProjectModalProps) {
  ...
  const [status, setStatus]         = React.useState<ProjectStatus>(project.status === "archived" ? "backlog" : project.status);
  const [sprints, setSprints]       = React.useState<string[]>(parseSprints(project.sprint));
  ...
  React.useEffect(() => {
    ...
    const match = members.find((m) => m.id === project.owner_id) ?? members.find((m) => m.name === project.owner_name);
    if (match) setOwner(match);
    const existingIds = Array.isArray(project.assignee_ids) ? project.assignee_ids : [];
    const existingNames = Array.isArray(project.assignees) ? (project.assignees as { id: string; name: string }[]).map((a) => a.name) : [];
    const matched = members.filter((m) => existingIds.includes(m.id) || existingNames.includes(m.name));
    if (matched.length) setAssignees(matched);
  }, [members, project.owner_id, project.owner_name, project.assignee_ids, project.assignees]);

  const quarter = dueDate ? computeQuarter(dueDate) : (project.quarter ?? "");
      ...
      <div className="w-full max-w-[560px] rounded-2xl bg-background shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <form onSubmit={handleSave} className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div>
            ...
            />
            {titleError && <p className="mt-1 text-xs text-red-500">Project title is required.</p>}
          </div>

          {/* Department */}
          <div>
            <label className="block text-sm font-semibold mb-2">Department</label>
            <Select value={department} onChange={setDepartment} options={DEPARTMENT_OPTIONS} placeholder="Select department…" />
          </div>

          {/* Owner */}
          <div>
            <label className="block text-sm font-semibold mb-2">Owner</label>
            <OwnerPicker members={members} loading={membersLoading} value={owner} onChange={setOwner} />
          </div>
          ...
          <div>
            ...
            <MultiSelect values={sprints} onChange={setSprints} options={SPRINT_OPTIONS} placeholder="Select sprint(s)…" position="up" />
          </div>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-border shrink-0">
          <p className="text-xs text-foreground-muted">
            {title.trim()
              ? `Will be saved as ${status.replace("_", " ")}`
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="submit" form="" disabled={saving || !title.trim()} onClick={handleSave}
